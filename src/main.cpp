...
GLuint Build_crosshair(); // Constrói um objeto 3D representando uma mira
...
int main(int argc, char* argv[])
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 600, "Makima Plushie Simulator", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
    ...
    GLuint vertex_array_object_id = Build_crosshair();
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        bezier_speed2 = 0.05f;
        if(beziert_bola < 1.0f && !way_back_bola)
            beziert_bola += delta_t * bezier_speed2;
        else{
            way_back_bola = true;
            beziert_bola -= delta_t * bezier_speed2;
            if(beziert_bola<=0 && way_back_bola){
                way_back_bola = false;
                beziert_bola = 0;
            }
        }
